...
callbacks = [
    ...
    ModelCheckpoint("models/gru_model.h5", save_best_only=True, monitor="val_accuracy", verbose=1),
    ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=3, min_lr=1e-5, verbose=1),
]

print("\n[GRU] Starting training ...")
history = model.fit(
    X_train,
    y_train_ohe,
    epochs=10,
...
print(f"\n[GRU] Test Accuracy: {acc*100:.2f}%  |  Test Loss: {loss:.4f}")
...
plt.tight_layout()
plt.savefig("outputs/gru_training_history.png", dpi=150, bbox_inches="tight")
plt.close()
print("[GRU] Training history saved to outputs/gru_training_history.png")
